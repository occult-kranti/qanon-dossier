...
| **`report.html`** | An **operator's report** that ties the project together: a step-by-step way to *read* it and to *run* it, an honest "what holds / what falls" account of what it reveals once the Audit is applied, and a **novel current-events conclusion** connecting the lab's *Delegated Closure* finding to the 2026 Epstein-files news cycle and the rise of AI "platform conspiracism" (conspiracy theories dressed as data analysis). The capstone page. |
| **`future.html`** | A page: a **novel measurement theory — "Delegated Closure"** — and its metric, the **Delegated-Closure Index (DCI)**, the first per-post, embedding-free, LLM-free, regex-computable measure of how much interpretive closure a drop withholds. Carries an honest results scorecard (4 passes, 1 downgrade, 1 rejected hypothesis, 1 blocked), the **synthetic-downstream-data disclosure**, a prior-research ledger, and a phased research roadmap. Reads `results/improved/dci_findings.json`. |
